...
class Room extends Scene {
    ...
    handleNavigation(x,y,player) {
        if ("requires" in this.engine.storyData.rooms[`{x}_{y}`]) {
            ...
            for (var req_ of requires) {
                if (player.item[req_]) {
                    ...
                    this.engine.show(`{req_} is needed to go in this direction`)
                    // give a "{item} neaded" message
                }
            }
    ...
    lock_puzzle(inventory, roomItems) {
